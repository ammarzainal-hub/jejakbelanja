...
function getCategoryTrend(month, year) {
  ...
  categories.forEach(function(cat) {
    result[cat.name] = {
      icon: cat.icon,
      months: [
        { month: getPreviousMonth(month, year, 2), total: 0, count: 0 },
        { month: getPreviousMonth(month, year, 1), total: 0, count: 0 },
        { month: { m: month, y: year }, total: 0, count: 0 }
      ]
    };
  });
...
function getBatchExpenseData(month, year) {
  return {
    ...
    categoryTrend : getCategoryTrend(month, year)
  };
}
